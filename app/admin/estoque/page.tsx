...
import { mapProductDoc } from "@/lib/firestore";



import type { Category, Product, ProductKit, Supplier } from "@/lib/types";
...
const kitSchema = z.object({
  ...
  label: z.string().min(2, "Informe o nome do kit"),



  sku: z.string().min(3, "Informe um SKU valido"),



  multiplier: z.coerce.number().int().min(1, "Multiplicador deve ser pelo menos 1")
...
const newProductSchema = z.object({
  ...
  unitPrice: z.coerce.number().min(0, "Preco invalido"),
...
function InventoryContent() {
  ...
  const enableKitSku = process.env.NEXT_PUBLIC_ENABLE_KIT_SKU === "true";
  ...
  const [salesInsights, setSalesInsights] = useState<Record<string, SalesInsight>>({});
  ...
  const [kitManager, setKitManager] = useState<{
  ...
  const sanitizeKit = (input: { label?: string | null; sku: string; multiplier: number }): ProductKit => ({

    label: input.label ? input.label.trim() : "",
  ...
  const kitForm = useForm<KitFormValues>({
  ...
  const handleOpenCreateKit = useCallback((product: Product) => {

    setKitManager({ product, kit: null, mode: "create" });

    kitForm.reset({ label: "", sku: "", multiplier: 1 });

  }, [kitForm]);



  const handleOpenEditKit = useCallback((product: Product, kit: ProductKit) => {

    setKitManager({ product, kit, mode: "edit" });
  ...
  const closeKitManager = useCallback(() => {
  ...
  const kitsFieldArray = useFieldArray({
  ...
  useEffect(() => {
    ...
    if (!enableKitSku || typeof window === "undefined") {
    ...
    try {
      ...
      const stored = window.localStorage.getItem("inventory-expanded-products");
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed)) {
          ...
          setExpandedProducts(parsed.filter((value: unknown): value is string => typeof value === "string"));
      ...
      console.error("Falha ao carregar estado expandido", error);
  ...
  useEffect(() => {
    ...
    try {
      ...
      window.localStorage.setItem("inventory-expanded-products", JSON.stringify(expandedProducts));

    } catch (error) {

      console.error("Falha ao persistir estado expandido", error);

    }

  }, [enableKitSku, expandedProducts]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const loadedProducts: Product[] = productsSnapshot.docs.map((doc: any) => mapProductDoc(doc));
        ...
        setExpandedProducts((prev) => prev.filter((id) => loadedProducts.some((product) => product.id === id)));
        ...
        const productIndexBySku = new Map(loadedProducts.map((product) => [product.sku, product.id]));
        ...
        const movementsQuery = firestore.query(
          ...
          firestore.where("timestamp", ">=", firestore.Timestamp.fromDate(startOfMonth))
        ...
        const movementsSnapshot = await firestore.getDocs(movementsQuery);
        ...
        const aggregated = new Map<string, { weekly: number; monthly: number }>();
        ...
        movementsSnapshot.docs.forEach((doc: any) => {



          const data = doc.data() as { productId?: string; sku?: string; qty?: number; type?: string; timestamp?: any };
          ...
          const productId =
            ...
            typeof data.productId === "string" && data.productId
              ...
              : typeof data.sku === "string"
                ...
                ? productIndexBySku.get(data.sku)
          ...
          const entry = aggregated.get(productId) ?? { weekly: 0, monthly: 0 };
        ...
        const insights: Record<string, SalesInsight> = {};
        ...
        loadedProducts.forEach((product) => {
          ...
          const entry = aggregated.get(product.id);



          const monthly = entry?.monthly ?? 0;



          const weekly = entry?.weekly ?? 0;



          const ideal = monthly > 0 ? monthly : 0;



          insights[product.id] = { weekly, monthly, ideal };
  ...
  const updateLists = (updatedProducts: Product[]) => {
    ...
    setSalesInsights((prev) => {
      ...
      const next: Record<string, SalesInsight> = {};
      ...
      updatedProducts.forEach((product) => {
        ...
        next[product.id] = prev[product.id] ?? { weekly: 0, monthly: 0, ideal: 0 };
  ...
  const handleUpdateProduct = async (id: string, changes: Partial<Product>) => {
    ...
    const estoqueMinimo =
      ...
      typeof changes.estoqueMinimo === "number" && Number.isFinite(changes.estoqueMinimo)



        ? Math.max(0, changes.estoqueMinimo)



        : typeof current.estoqueMinimo === "number"



          ? Math.max(0, current.estoqueMinimo)
    ...
    try {
      ...
      await firestore.updateDoc(productRef, {
        ...
        estoqueMinimo: typeof updated.estoqueMinimo === "number" ? updated.estoqueMinimo : 0
  ...
  const persistKitChanges = async (productId: string, kits: ProductKit[]) => {

    const sanitized = kits.map(sanitizeKit);

    const kitSkus = sanitized.map((kit) => kit.sku);

    const bundle = await ensureFirebase();

    const { firestore } = bundle;

    const productRef = firestore.doc(bundle.db, "products", productId);
 console.log({ productId, kits: sanitized, kitSkus });
    await firestore.updateDoc(productRef, {
    ...
    return { sanitized, kitSkus };

  };



  const handleKitSubmit = kitForm.handleSubmit(async (values) => {
    ...
    const { product, kit, mode } = kitManager;

    const sanitizedSku = values.sku.trim();

    const sanitizedLabel = values.label.trim();

    const multiplier = Math.max(1, Math.floor(values.multiplier));
    ...
    if (sanitizedSku === product.sku) {
      ...
      toast.error("SKU do kit nao pode ser igual ao SKU do produto pai.");
    ...
    if (products.some((item) => item.id !== product.id && item.sku.toLowerCase() === skuLower)) {

      toast.error("Ja existe um produto com este SKU.");
    ...
    const existingKits = Array.isArray(product.kits) ? product.kits : [];



    const duplicate = existingKits.some((existingKit) =>

      existingKit.sku.toLowerCase() === skuLower &&

      (mode === "create" || existingKit.sku.toLowerCase() !== (kit?.sku?.toLowerCase() ?? ""))
    ...
    if (duplicate) {
      ...
      toast.error("Ja existe um kit com este SKU.");
    ...
    const nextKits =
      ...
      mode === "edit" && kit
        ...
        ? existingKits.map((existingKit) =>

            existingKit.sku === kit.sku

              ? { label: sanitizedLabel, sku: sanitizedSku, multiplier }

              : existingKit

          )

        : [...existingKits, { label: sanitizedLabel, sku: sanitizedSku, multiplier }];



    try {

      const { sanitized, kitSkus } = await persistKitChanges(product.id, nextKits);
      ...
      toast.success(mode === "edit" ? "Kit atualizado." : "Kit criado.");
  ...
  const handleDeleteKit = async (product: Product, kit: ProductKit) => {

    try {

      const remaining = (product.kits ?? []).filter((item) => item.sku !== kit.sku);

      const { sanitized, kitSkus } = await persistKitChanges(product.id, remaining);
  ...
  const handleGenerateKitLabel = (product: Product, kit: ProductKit) => {

    const item: LabelItem = {

      sku: kit.sku,

      name: kit.label?.trim().length ? kit.label : product.name
  ...
  const handleCreateProduct = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      const sanitizedKits = enableKitSku && Array.isArray(values.kits) ? values.kits.map(sanitizeKit) : [];
      ...
      if (enableKitSku) {
        ...
        for (const kit of sanitizedKits) {

          const lower = kit.sku.toLowerCase();

          if (!kit.sku) {

            toast.error("Informe um SKU valido para o kit.");

            return;

          }

          if (kit.sku === values.sku) {
          ...
          if (products.some((item) => item.sku.toLowerCase() === lower)) {
      ...
      const kitSkus = sanitizedKits.map((kit) => kit.sku);
      ...
      if (enableKitSku) {
        ...
        productData.kits = sanitizedKits;

        productData.kitSkus = kitSkus;
      ...
      updateLists([...products, newProduct]);
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        ...
        toast.error("Formato invalido de arquivo");
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <EditableTable
          ...
          enableKits={enableKitSku}



          expandedProductIds={enableKitSku ? expandedProducts : undefined}



          onToggleExpand={enableKitSku ? toggleExpandedProduct : undefined}



          onCreateKit={enableKitSku ? handleOpenCreateKit : undefined}



          onEditKit={enableKitSku ? handleOpenEditKit : undefined}



          onDeleteKit={enableKitSku ? handleDeleteKit : undefined}



          onGenerateKitLabel={enableKitSku ? handleGenerateKitLabel : undefined}
          ...
          <form onSubmit={handleCreateProduct} className="space-y-4">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <Input id="estoqueMinimo" type="number" min="0" {...form.register("estoqueMinimo")} />



                {form.formState.errors.estoqueMinimo ? (



                  <p className="text-sm text-red-500">{form.formState.errors.estoqueMinimo.message}</p>
            ...
            {enableKitSku ? (
              ...
              <div className="space-y-3 rounded-xl border border-slate-200 p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <h4 className="text-sm font-semibold text-slate-700">Kits</h4>
                  ...
                  <Button
                    ...
                    onClick={() => kitsFieldArray.append({ label: "", sku: "", multiplier: 1 })}
                ...
                {kitsFieldArray.fields.length ? (

                  <div className="space-y-3">

                    {kitsFieldArray.fields.map((field, index) => {

                      const kitErrors = form.formState.errors.kits?.[index];

                      return (

                        <div key={field.id} className="space-y-3 rounded-lg border border-slate-200 p-3">

                          <div className="grid gap-3 md:grid-cols-2">

                            <div className="space-y-1">

                              <Label htmlFor={`kit-label-${index}`}>Nome</Label>

                              <Input

                                id={`kit-label-${index}`}

                                {...form.register(`kits.${index}.label` as const)}

                              />

                              {kitErrors?.label ? (

                                <p className="text-xs text-red-500">{kitErrors.label.message}</p>
                            ...
                            <div className="space-y-1">
                              ...
                              <Label htmlFor={`kit-sku-${index}`}>SKU</Label>

                              <Input

                                id={`kit-sku-${index}`}

                                {...form.register(`kits.${index}.sku` as const)}

                              />

                              {kitErrors?.sku ? (

                                <p className="text-xs text-red-500">{kitErrors.sku.message}</p>
                            ...
                            <div className="space-y-1">
                              ...
                              <Label htmlFor={`kit-multiplier-${index}`}>Multiplicador</Label>

                              <Input

                                id={`kit-multiplier-${index}`}

                                type="number"

                                min={1}

                                {...form.register(`kits.${index}.multiplier` as const, { valueAsNumber: true })}

                              />

                              {kitErrors?.multiplier ? (

                                <p className="text-xs text-red-500">{kitErrors.multiplier.message}</p>
                          ...
                          <div className="flex justify-end">
                            ...
                            <Button
                              ...
                              onClick={() => kitsFieldArray.remove(index)}
                  ...
                  <p className="text-sm text-slate-500">Nenhum kit adicionado ainda.</p>
      ...
      <Dialog open={Boolean(kitManager)} onOpenChange={(open) => {
        ...
        <DialogContent>
          ...
          <DialogHeader>
            ...
            <DialogTitle>{kitManager?.mode === "edit" ? "Editar kit" : "Novo kit"}</DialogTitle>

            <DialogDescription>

              {kitManager ? `Produto: ${kitManager.product.name}` : ""}
          ...
          <form onSubmit={handleKitSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Label htmlFor="kit-label">Nome</Label>

              <Input id="kit-label" {...kitForm.register("label")} />

              {kitForm.formState.errors.label ? (

                <p className="text-sm text-red-500">{kitForm.formState.errors.label.message}</p>
            ...
            <div className="space-y-2">
              ...
              <Input id="kit-sku" {...kitForm.register("sku")} />

              {kitForm.formState.errors.sku ? (

                <p className="text-sm text-red-500">{kitForm.formState.errors.sku.message}</p>
            ...
            <div className="space-y-2">
              ...
              <Label htmlFor="kit-multiplier">Multiplicador</Label>

              <Input id="kit-multiplier" type="number" min={1} {...kitForm.register("multiplier", { valueAsNumber: true })} />

              {kitForm.formState.errors.multiplier ? (

                <p className="text-sm text-red-500">{kitForm.formState.errors.multiplier.message}</p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button type="button" variant="outline" onClick={closeKitManager}>

                Cancelar

              </Button>

              <Button type="submit" disabled={kitForm.formState.isSubmitting}>

                {kitForm.formState.isSubmitting

                  ? "Salvando..."

                  : kitManager?.mode === "edit"
